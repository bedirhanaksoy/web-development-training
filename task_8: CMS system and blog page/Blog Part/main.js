...
function updateFilteredContentDiv(currentPage, category_id){
    const url = "getFilteredTopicsByPageNumber.php";
                        ...
                        <div class="top-div">
                            ...
                            <div class="topic-content" id="${"Div"+currentElement}" style="display:block">
                                ...
                                <form action="" method="post" onsubmit="return verifyComment(${topic.TopicID})">
                                    <textarea id="comment_area_id_${topic.TopicID}" type="text" class="custom-placeholder comment_field" name="comment_field" id="comment_field" placeholder="Enter a comment" ></textarea><br>
...
function getFilteredTopicPages(category_id) {
    const url = "readFilteredTopicsPageNumber.php";
...
function performCreateComment(usersID, topicID) {
    ...
    const url = "createNewComment.php";
...
function getCommentsAndFillCommentFields(topicID){
        ...
        .then(data => {
            if (data.status === 'success') {
                ...
                if (comments.length > 0 && comments != -1) {
                    document.getElementById('comment-box-'+topicID).innerHTML = ""; 
                    ...
                    comments.forEach(comment => {
                        ...
                        <div class="comment-content">
                            <div class="publish-box">     ${comment.publish_date}
                            </div><br>
                            <article>${comment.content}</article><br>
                        </div>
                        `;
